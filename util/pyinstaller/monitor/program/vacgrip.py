...
class VacGripData:
    ...
    flag: npt.NDArray[np.bool8]
    accelerometer: npt.NDArray[np.float_]
    gyroscope: npt.NDArray[np.float_]
    temperature: npt.NDArray[np.float_]
    loadcell: npt.NDArray[np.float_]
    microphone: npt.NDArray[np.float_]
...
def load_rawdata(file_path: str) -> VacGripRawData:
    ...
    return VacGripRawData(
        ...
        flag=np.array(flag, dtype=np.bool8),
